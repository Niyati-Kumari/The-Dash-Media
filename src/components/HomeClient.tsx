...
const VerticalShowcase = dynamic(() => import('./VerticalShowcase'), { ssr: false });
const BrandsSection = dynamic(() => import('./BrandsSection'), { ssr: false });
const ReviewsSection = dynamic(() => import('./ReviewsSection'), { ssr: false });
const CareersSection = dynamic(() => import('./CareersSection'), { ssr: false });
...
export default function HomeClient() {
  ...
  return (
    <div className="bg-[#050505] text-white selection:bg-white selection:text-black font-sans" style={{ overflowX: 'clip' }}>
      <Header />
          ...
          <div className="mt-16 md:mt-24 pt-6 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="flex items-center gap-4">
               <div className="relative h-5 w-24">
                 <Image src="/logo-transparent.png" alt="Dash Media" fill sizes="96px" className="object-contain invert brightness-0" />
               </div>
  ...
  return (
    ...
    >
      <div className={`absolute inset-0 z-0 transition-all duration-700 ${isHovered ? "opacity-100 scale-100" : "opacity-20 scale-110"}`}>
        <video 
          autoPlay={isHovered} // simple way to start/stop
          muted 
          loop 
          playsInline 
