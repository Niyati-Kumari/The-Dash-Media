    ...
    <>
      <header
        className={`fixed top-0 left-0 w-full z-50 transition-all duration-700 flex items-center justify-between px-6 sm:px-10 lg:px-20 xl:px-24 py-3 md:py-5 ${
          isScrolled
            ? "bg-black/20 backdrop-blur-xl border-b border-white/5 py-3 md:py-4"
            : "bg-transparent"
        }`}
      >
        ...
        >
          <span
            className={`text-xs md:text-sm font-bold uppercase tracking-[0.3em] transition-all duration-500 ${isMenuOpen ? "text-white" : "text-white/70 group-hover:text-white"}`}
          >
            ...
            <nav className="flex flex-col items-center gap-6 sm:gap-8 md:gap-12">
              {[
                ...
                <motion.a
                  ...
                  animate={{
                    opacity:
                      hoveredIndex === null ? 1 : hoveredIndex === i ? 1 : 0.3,
                    y: 0,
