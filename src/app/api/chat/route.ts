  ...
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      return NextResponse.json(
        {
          reply:
            "⚠️ API Key is missing. Please add GEMINI_API_KEY to your .env.local file to enable the real AI chatbot.",
        },
        { status: 200 },
      );
    }
...
${dashmediaContext}
    ...
    if (
      ...
      return NextResponse.json(
        {
          reply:
            "⚠️ The API key provided is invalid. Please get a new key from aistudio.google.com and update the .env.local file.",
        },
        { status: 500 },
      );
    }
